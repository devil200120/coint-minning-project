...
const Sidebar = ({ isOpen, onClose, isCollapsed }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`shrink-0 p-3 border-t border-white/10 bg-slate-900/95 backdrop-blur-sm ${
          isCollapsed ? "lg:p-2" : ""
        }`}
      >
        <button
          onClick={handleLogout}
          className={`group relative w-full flex items-center gap-3 px-3 py-3 text-slate-400 rounded-xl hover:bg-red-500/10 hover:text-red-400 transition-all duration-200 ${
            isCollapsed ? "lg:justify-center lg:px-0" : ""
          }`}
        >
